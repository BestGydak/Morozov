...
class Report:
    ...
    def generate_charts(self, _salary_dynamic, _count_dynamic,
        ...
        self.add_salary_dynamic_chart(figure, _salary_dynamic, _selected_salary_dynamic, _vacancy_name)
        self.add_count_dynamic_chart(figure, _count_dynamic, _selected_count_dynamic, _vacancy_name)
        self.add_top_cities_chart(figure, _top_cities, _vacancy_name)
        self.add_top_cities_count_chart(figure, _top_cities_count, _vacancy_name)

        plt.subplots_adjust(wspace=0.5, hspace=0.5)
        plt.show()
        figure.savefig('graph.png')

    def add_salary_dynamic_chart(self, figure, _salary_dynamic, _selected_salary_dynamic, _vacancy_name):
        subplot = figure.add_subplot(2, 2, 1)
        x = list(_salary_dynamic)
        y1 = [_salary_dynamic[k] for k in x]
        y2 = [_selected_salary_dynamic[k] for k in x]
        subplot.bar([i - 0.2 for i in x], y1, label='средняя з/п', width=0.4)
        subplot.bar([i + 0.2 for i in x], y2, label=f'з/п {_vacancy_name}', width=0.4)
        subplot.set_xticks(x, fontsize=8)
        subplot.tick_params(axis='x', labelrotation=90)
        subplot.grid(axis='y')
        subplot.set(title='Уровень зарплат по годам')

    def add_count_dynamic_chart(self, figure, _count_dynamic, _selected_count_dynamic, _vacancy_name):
        subplot = figure.add_subplot(2, 2, 2)
        x = list(_count_dynamic)
        y1 = [_count_dynamic[k] for k in x]
        y2 = [_selected_count_dynamic[k] for k in x]
        subplot.bar([i - 0.2 for i in x], y1, label='Количество вакансий', width=0.4)
        subplot.bar([i + 0.2 for i in x], y2, label=f'Количество вакансий {_vacancy_name}', width=0.4)
        subplot.set_xticks(x, fontsize=8)
        subplot.tick_params(axis='x', labelrotation=90)
        subplot.grid(axis='y')
        subplot.set(title='Количество вакансий по годам')

    def add_top_cities_chart(self, figure, _top_cities_chart, _vacancy_name):
        subplot = figure.add_subplot(2, 2, 3)
        x = list(_top_cities_chart)[:10]
        y = [_top_cities_chart[k] for k in x]
    ...
    def add_top_cities_count_chart(self, figure, _top_cities_count, _vacancy_name):
        subplot = figure.add_subplot(2, 2, 4)
        x = list(_top_cities_count)[:10][::-1]
        y = [_top_cities_count[k] for k in x]
        x.append('Другие')
        y.append(1 - sum(y))
        subplot.pie(y, labels=x)
        subplot.set(title='Доля вакансий по городам')

    def generate_excel(self, salary_dynamic, count_dynamic,
                       selected_salary_dynamic ,selected_count_dynamic,
                       top_cities, top_cities_count, vacancy_name):
        ws = self._wb.active
        ws.title = 'Статистика по годам'
        header = ['Год', 'Средняя зарплата', f'Средняя зарплата - {vacancy_name}', 'Количество вакансий',
                  f'Количество вакансий - {vacancy_name}']
        ws.append(header)
        for year in salary_dynamic:
            row = [year]
            if year in salary_dynamic:
                row.append(salary_dynamic[year])
            if year in count_dynamic:
                row.append(salary_dynamic[year])
            if year in selected_salary_dynamic:
                row.append(salary_dynamic[year])
            if year in selected_count_dynamic:
                row.append(salary_dynamic[year])
            if year in top_cities:
                row.append(salary_dynamic[year])
            if year in top_cities_count:
                row.append(salary_dynamic[year])
            ws.append(row)
        ...
        ws.append(header)
        for city, _ in zip(top_cities_count, range(10)):
            row = [city]
            row.append(top_cities[city])
            row.append('')
            row.append(city)
            row.append(top_cities_count[city])
            ws.append(row)
...
report = Report(font, header_font, border)
#report.generate_excel(salary_dynamic, count_dynamic, selected_salary_dynamic,
#                      selected_count_dynamic, top_cities, top_cities_count, vacancy_name)

report.generate_charts(salary_dynamic, count_dynamic, selected_salary_dynamic,
                       selected_count_dynamic, top_cities, top_cities_count, vacancy_name)
